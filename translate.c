...
*/
tr_access_t tr_alloc_local(tr_level_t level, int escape){
...
static ir_expr_t un_ex(tr_expr_t expr){ 
   ...
   switch (expr->kind){
      ...
      case TR_CX: {
         ...
         return ir_eseq_expr( ir_seq_stmt(
                                 vlist(
                                    5, 
                                    ir_move_stmt(
                                       ir_tmp_expr(tmp), 
...
tr_expr_t tr_rel_expr(int op, tr_expr_t left, tr_expr_t right){
   ...
   return tr_cx(list(&stmt->u.cjump.t, NULL), list(&stmt->u.cjump.f, NULL), stmt);
}

tr_expr_t tr_string_rel_expr(int op, tr_expr_t left, tr_expr_t right){

   ir_expr_t expr = fr_external_call("_CompareString", list(left, list(right, NULL)));
...
tr_expr_t tr_record_expr(list_t fields, int size){
   ...
   return tr_ex( ir_eseq_expr( ir_seq_stmt(list(ir_move_stmt(addr, alloc), q)), addr));
}

tr_expr_t tr_array_expr(tr_expr_t size, tr_expr_t init){
   return tr_ex(fr_external_call( "_InitArray", list(un_ex(size), list(un_ex(init), NULL))));
}
...
tr_expr_t tr_for_expr(tr_access_t access, tr_expr_t low, tr_expr_t high, tr_expr_t body){
...
/*   ir_expr_t fp = ir_tmp_expr(fr_fp());
   while (level != access->level){
      ...
      fp = ir_mem_expr(ir_binop_expr( IR_PLUS, fp, ir_const_expr(fr_offset(fr_access))));
      level = level->parent;
   } 
